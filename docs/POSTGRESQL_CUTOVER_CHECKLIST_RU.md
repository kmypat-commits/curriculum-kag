...
## Целевой рабочий режим

PostgreSQL является целевым основным режимом после приёмки Docker, миграций и
endpoint-контрактов:
...
Пока Docker Desktop/CLI недоступен, `start.ps1 -Database postgres` должен
быстро завершиться ясной ошибкой, а не тихо переключаться на SQLite. Такой
режим не считается завершённым переходом на PostgreSQL.
...
Расширенная приёмка дополнительно строит новые бакалаврские, магистерские и
докторские A/B/C с нуля и поэтому выполняется дольше:
