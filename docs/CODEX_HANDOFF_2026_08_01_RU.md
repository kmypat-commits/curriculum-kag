...
- 2026-08-02: scheduler now aggregates `typical_semester` by the selected EPVO groups/directions and education level using a median before placement, filtering values outside `[1, total_semesters]`, then applies a conservative load/prerequisite-safe semester repair. The audit now reads the same EPVO-ID key used for aggregation instead of the local course primary key. Corrected fresh transactional audit of seven A-variants produced adjusted EPVO provenance 0.9964 and semester alignment 0.619; the historical quality-eligible set reaches 0.635. This is structural validation, not blinded expert evaluation.
- 2026-08-02: a scoped-semester-stability tie-break was tested in candidate ranking. Full seven-program fresh holdout stayed unchanged at provenance 0.9964 and semester alignment 0.619, with no new hard violations; the change is retained as a conservative future-facing tie-break, not reported as a metric gain.
