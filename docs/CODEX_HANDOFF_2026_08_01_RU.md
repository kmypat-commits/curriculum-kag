...
- 2026-08-02: scheduler now aggregates `typical_semester` by the selected EPVO groups/directions and education level using a median before placement, filtering values outside `[1, total_semesters]`. Fresh transactional external audit of seven A-variants produced adjusted EPVO provenance 0.9964 and semester alignment 0.627; this is structural validation, not blinded expert evaluation.
