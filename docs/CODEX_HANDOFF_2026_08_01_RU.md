...
- 2026-08-02: scheduler now aggregates `typical_semester` by the selected EPVO groups/directions and education level using a median before placement, filtering values outside `[1, total_semesters]`, then applies a conservative load/prerequisite-safe semester repair. The audit now reads the same `approved_course_id` key used for aggregation instead of the local course primary key and excludes missing semester observations from means. Corrected fresh transactional audit of seven A-variants produced EPVO provenance 0.9964 and strict scoped-semester alignment 0.367 (any-source 0.367); the historical quality-eligible set reaches 0.200 on the strict scoped metric (0.229 when any valid source is allowed). These are structural comparisons with the EPVO repository, not blinded expert evaluation.
- 2026-08-02: a scoped-semester-stability tie-break was tested in candidate ranking. Full seven-program fresh holdout stayed unchanged at provenance 0.9964 and strict semester alignment 0.367, with no new hard violations; the change is retained as a conservative future-facing tie-break, not reported as a metric gain.
