...
Full listwise CUDA-кандидат (12 000 train-групп, 55 835 положительных,
195 200 отрицательных, graded expert loss) проверен на независимых 80/80
программах. Dynamic train-only EPVO memory выбрала по validation веса
0,3/0,4; test Recall@10=0,6398 против 0,5960 baseline (+0,0438),
MRR=0,6543, nDCG@10=0,5799. Улучшение воспроизводимо относительно
предыдущего кандидата 0,6271, но до порога 0,80 не дотягивает и потому не
продвигается в production.

Расширенный frozen контроль на 160/160 programme-disjoint программах
подтвердил направление: Recall@10=0,6314 против 0,5866 baseline (+0,0448),
MRR=0,6447, nDCG@10=0,5675. Разница с 80/80 (0,6398) не меняет вывода:
улучшение устойчиво, но acceptance-порог 0,80 ещё не достигнут.

Контроль прежней 40k-модели на идентичном 160/160 split дал Recall@10=0,6191
(baseline 0,5665, delta +0,0526), MRR=0,6358 и nDCG@10=0,5566. Listwise
кандидат 0,6314 лучше по абсолютному Recall@10, но оба результата ниже
порога promotion 0,80; production-рейтинг остаётся прежним.

В all-labels пилоте выявлен NaN-риск: declared edges без expert vote
представлялись как score=0 и могли дать нулевой listwise denominator. Обработка
исправлена без изменения исходных EPVO-данных: `None` трактуется как
unlabelled positive, а явный ноль получает конечный минимальный вес. Добавлен
регрессионный тест; backend regression после правки — 92/92. Неудачный
первый запуск остановлен до checkpoint и не учитывается.
...
метрика v3 не выдаётся за измеренный результат. Дополнительно CrossEncoder
считает пары по одному LO, а не создаёт весь декартов LO×дисциплины; это
ограничивает пиковую память на больших программах без изменения формулы
оценки. Контрольный v3-20 на 20/20 held-out программах завершился без
ошибок: test Recall@10=0,5461 против 0,5422 baseline (+0,0039), MRR=0,5258;
результат диагностический и не продвигается. Отдельный full weighted-memory
